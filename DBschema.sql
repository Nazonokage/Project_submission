...
CREATE TABLE "documentation_field_templates" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "documentation_field_templates_slot_id_field_key_key" UNIQUE("slot_id","field_key"),
	CONSTRAINT "documentation_field_templates_field_type_check" CHECK ((field_type = ANY (ARRAY['text'::text, 'textarea'::text, 'url'::text, 'date'::text])))
);
...
CREATE TABLE "tasks" (
	...
	"deleted_at" timestamp with time zone,
	"sort_order" integer DEFAULT 0 NOT NULL,
	CONSTRAINT "tasks_status_check" CHECK ((status = ANY (ARRAY['planning'::text, 'in_progress'::text, 'review'::text, 'done'::text])))
);
...
CREATE INDEX "idx_activity_class_action" ON "activity_log" ("class_id","action","created_at");
CREATE UNIQUE INDEX "classes_pkey" ON "classes" ("id");
CREATE INDEX "idx_classes_prof_id" ON "classes" ("prof_id");
CREATE UNIQUE INDEX "documentation_field_templates_pkey" ON "documentation_field_templates" ("id");
CREATE UNIQUE INDEX "documentation_field_templates_slot_id_field_key_key" ON "documentation_field_templates" ("slot_id","field_key");
CREATE INDEX "idx_doc_field_templates_slot" ON "documentation_field_templates" ("slot_id","sort_order");
...
CREATE INDEX "idx_project_updates_task" ON "project_updates" ("task_id","created_at");
...
CREATE INDEX "idx_tasks_assignee" ON "tasks" ("assignee_student_id","status");
CREATE INDEX "idx_tasks_deleted_at" ON "tasks" ("deleted_at");
CREATE INDEX "idx_tasks_group" ON "tasks" ("group_id");
CREATE INDEX "idx_tasks_title_status" ON "tasks" ("title_id","status");
CREATE UNIQUE INDEX "tasks_pkey" ON "tasks" ("id");
CREATE INDEX "idx_title_reports_class_slot" ON "title_reports" ("class_id","slot_id");
CREATE INDEX "idx_title_reports_deleted_at" ON "title_reports" ("deleted_at");
CREATE INDEX "idx_title_reports_group" ON "title_reports" ("group_id");
CREATE INDEX "idx_title_reports_project_update" ON "title_reports" ("project_update_id");
...
ALTER TABLE "documentation_field_templates" ADD CONSTRAINT "documentation_field_templates_slot_id_fkey" FOREIGN KEY ("slot_id") REFERENCES "project_slots"("id") ON DELETE CASCADE;
...
ALTER TABLE "project_updates" ADD CONSTRAINT "project_updates_task_id_fkey" FOREIGN KEY ("task_id") REFERENCES "tasks"("id") ON DELETE SET NULL;
...
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_assignee_student_id_fkey" FOREIGN KEY ("assignee_student_id") REFERENCES "students"("id") ON DELETE SET NULL;
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_class_id_fkey" FOREIGN KEY ("class_id") REFERENCES "classes"("id") ON DELETE CASCADE;
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_group_id_fkey" FOREIGN KEY ("group_id") REFERENCES "groups"("id") ON DELETE CASCADE;
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_slot_id_fkey" FOREIGN KEY ("slot_id") REFERENCES "project_slots"("id") ON DELETE CASCADE;
ALTER TABLE "tasks" ADD CONSTRAINT "tasks_title_id_fkey" FOREIGN KEY ("title_id") REFERENCES "titles"("id") ON DELETE CASCADE;
...
ALTER TABLE "title_reports" ADD CONSTRAINT "title_reports_project_update_id_fkey" FOREIGN KEY ("project_update_id") REFERENCES "project_updates"("id") ON DELETE SET NULL;
